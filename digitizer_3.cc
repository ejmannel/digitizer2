...
int digitizer(int pkt,  char *file, int nsamples, int nmodules) {
  ...
  while ( (evt = it->getNextEvent())) {
	...
	for (imodule = 0; imodule < nmodules; imodule++) {
	  ...
	  if (verbose) { 
	    ...
	    std::cout << "Module event number: " << adc->FEMEvtNum[imodule] << std::endl;	    std::cout << "             Module slot number: " 
	...
	if (firstPacket == 1) { 
	  ...
	  // if ((n_events%65536 != (adc->eventNumber+1)%65536 && n_events < 20)) {
	  //   errorInPkt |=  0x0002;
	  // }
	  if ((adc->eventNumber == 0 && EventNum_last != 0x0000FFFF) ||
	    ...
	    event_err[0]++;
	    errorInPkt |= 0x0004;
	    if (adc->eventNumber == EventNum_last) 
	      duplicateEvent[0]++;
	    else 
	      skippedEvent[0]++;
	  }
	  ...
	  for (imodule = 0; imodule < nmodules; imodule ++) {
	    ...
	    if ((adc->eventNumber+1)%65536 != adc->FEMEvtNum[imodule]) { 
	      event_err2[imodule]++;
	      errorInPkt |= 0x0008;
	    }
		...
		(adc->FEMEvtNum[imodule] != 0 && 
		 adc->FEMEvtNum[imodule] - evtNum_last[imodule] != 1)) {
	      event_err[imodule+1]++;
	      errorInPkt |= 0x0010;
	      if (adc->FEMEvtNum[imodule] == evtNum_last[imodule]) 
		duplicateEvent[imodule+1]++;
	      else 
		skippedEvent[imodule+1]++;
	    }
	  ...
	  if (adc->FEMEvtNum[0] != adc->FEMEvtNum[1]) {
	    event_err3[0]++;
	    errorInPkt |= 0x0040;
	  }
	  if (adc->FEMEvtNum[0] != adc->FEMEvtNum[2]) { 
	    event_err3[1]++;
	    errorInPkt |= 0x0040;
	  }
	if (adc->FEMEvtNum[2] != adc->FEMEvtNum[2]) {
	    event_err3[2]++;
	    errorInPkt |= 0x0040;
	  }

	  if (FEMBCO_Delta[0] != FEMBCO_Delta[1]) {
	    BCO_err2[0]++;
	    bco_err++;
	  }
	  if (FEMBCO_Delta[0] != FEMBCO_Delta[2]){
	    BCO_err2[1]++;
	    bco_err++;
	  }
	  if (FEMBCO_Delta[1] != FEMBCO_Delta[2]) {  
	    BCO_err2[2]++;
	    bco_err++;
	  }
	  ...
	  if (errorInPkt) {
	    if (verbose) { 
	      ...
	      fprintf(errorFile, 
		      ...
		      n_events, n_recs, adc->eventNumber, EventNum_last);
  ...
  fprintf(errorFile,"\n");
  fprintf(errorFile, "Duplicate Number Errors (Global: FEM 0: FEM 1: FEM2)");
  for (int i = 0; i < nmodules+1; i++) 
    fprintf(errorFile, ": %d ", duplicateEvent[i]);
  fprintf(errorFile,"\n");
  fprintf(errorFile, "Skipped Number Errors (Global: FEM 0: FEM 1: FEM2)");
  for (int i = 0; i < nmodules+1; i++) 
    fprintf(errorFile, ": %d ", skippedEvent[i]);
  fprintf(errorFile,"\n");
